...
const scheduleFilenames = Object.freeze(Object.fromEntries(Object.keys(EntryType).map(k =>
    [k, `cfm-${k.toLowerCase()}.bin`]
))) as Readonly<Record<EntryType, string>>;

const entryTypeRegex = new RegExp(`^(?:HORA)?(${Object.keys(EntryType).join("|")}).+\\.pdf$`);
...
export async function getCfmSchedule(): Promise<Map<string, Subject>> {
    ...
    const scheduleFilePaths = htmlScheduleRows.reduce((schedules, row) => {
        ...
        const type = fileName?.match(entryTypeRegex)?.[1] as EntryType | undefined;
        ...
        return schedules;
    }, {} as Record<EntryType, ScheduleFilePath>);
...
async function getSubjects(csv: string[][], entryType: EntryType): Promise<Map<string, Subject>> {
...
function prepareSubjectSchedule(row: string[], sections: number, entryType: EntryType): PreparedSchedule[] {
